...
SELECT 'NOMBRE', nombre, 'CARGO', cargo_emp
FROM empleados;
...
SELECT nombre, 'Valor total a pagar', sal_emp+comision_emp+500
FROM empleados
WHERE id_depto = 3000
ORDER BY nombre;
...
SELECT nombre, sal_emp, comision_emp, 'Salario total', sal_emp+comision_emp
FROM empleados
...
SELECT *
FROM empleados
WHERE comision_emp <= (0.30*sal_emp);
...
SELECT max(sal_emp), min(sal_emp), 'DIFERENCIA', (max(sal_emp)-min(sal_emp))
FROM empleados;
...
SELECT departamentos.id_depto, departamentos.nombre_depto, COUNT(*) as 'num_empleados'
FROM empleados, departamentos
WHERE departamentos.id_depto = empleados.id_depto
GROUP BY departamentos.nombre_depto
HAVING COUNT(*) >= 3;

SELECT empleados.cod_jefe, empleados.nombre, COUNT(*) as 'num_empleados'
FROM empleados, departamentos
WHERE departamentos.cod_director = empleados.cod_jefe
GROUP BY departamentos.nombre_depto
HAVING COUNT(*) >= 2;
...
SELECT nombre, id_depto, sal_emp
FROM empleados
